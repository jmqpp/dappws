const XianWalletUtils = {
	...
	walletSendMessage: function(message, detail) {
		if(socket != null) {
			if(detail != undefined) socket.send(JSON.stringify({msg: message, detail: detail.detail}));
			else socket.send(JSON.stringify({msg: message}));
		} else {
    ...
    init: function(rpcUrl, port, addr) {
      ...
      if(port != undefined) {
				...
				socket.addEventListener("message", (ev) => {
					...
					switch(event.msg) {
						case 'xianWalletInfo': this.walletInfoHandle(event.detail); break;
						case 'xianWalletSignMsgResponse': this.walletSignMsgResponseHandle(event.detail); break;
						case 'xianWalletTxStatus': this.walletTxStatusHandle(event.detail); break;
						case 'xianReady': this.walletReadyHandle(); break;
					}
				});
    ...
    signMessage: async function(message) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.walletSendMessage('xianWalletSignMsg', {detail: {message: message}});
        });
    },
    ...
    sendTransaction: async function(contract, method, kwargs, stampLimit=0) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.walletSendMessage('xianWalletSendTx', {detail: eventDetail});
        });
    },
